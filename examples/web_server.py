...
CLIENT_DIRECTORY = Path(__file__).resolve().parents[1] / "ui" / "api"
CLIENT_FILES = {
    "/client/": ("index.html", "text/html; charset=utf-8"),
    "/client/app.js": ("app.js", "text/javascript; charset=utf-8"),
    "/client/styles.css": ("styles.css", "text/css; charset=utf-8"),
}
...
class MinesweeperRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in CLIENT_FILES:
            filename, content_type = CLIENT_FILES[path]
    ...
    def _write_static_file(self, filename: str, content_type: str) -> None:
        """Write one allowlisted API client file."""
        try:
            body = (CLIENT_DIRECTORY / filename).read_bytes()
        except OSError:
            self._write_error(HTTPStatus.NOT_FOUND, "Client file not found.")
            return
